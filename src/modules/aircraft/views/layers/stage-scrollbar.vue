<template>
    <k-layer>
        <template v-if="store.aircrafts.length">
            ...
            <app-k-text
                :config="{
                    text: config.aircraft.number,
                    x: 0,
                    y: config.stageScrollbarLayer.aircraftNumber.y,
                    width: config.fixedAside.width,
                    fontSize: config.stageScrollbarLayer.aircraftNumber.fontSize
                }"
            />
            <app-k-text
                :config="{
...
export default {
    ...
    created() {
        ...
        window.addEventListener("resize", _.throttle(this.onWindowResizeHandler, throttleInterval));
