            ...
            this.store.aircrafts.forEach((aircraft, index) => {
                series.push({
                    ...
                    data: _.map(aircraft.flights, (flight) => ({
                        ...
                        flight,
                        y: index,
                        color: flight.serviceMode ? config.serviceModes[flight.serviceMode.id].color : "transparent"
                    }))
                });
        ...
        onRender(chart) {
            ...
            chart.series.forEach((serie) => {
                serie.points.forEach((point) => {
                    ...
                    if (!point.options.flight.serviceMode) {
                        this.addArrowUp({
                            chart,
                        ...
                        this.addText({
                            chart,
                            text: config.serviceModes[point.options.flight.serviceMode.id].text,
        ...
        addArrowUp({ chart, x, y }) {
            ...
            const arrow = chart.renderer
                .path([
                    "M",
                    arrowMainX,
                    arrowMainY + config.arrow.mainHeight,
                    "L",
                    arrowMainX,
                    arrowMainY,
                    "L",
        ...
        addArrowDown({ chart, x, y }) {
            ...
            const arrow = chart.renderer
                .path([
                    ...
                    "L",
                    arrowMainX - config.arrow.pointerSize,
                    arrowMainY + config.arrow.mainHeight - config.arrow.pointerSize,
                    "M",
                    arrowMainX,
                    arrowMainY + config.arrow.mainHeight,
                    "L",
